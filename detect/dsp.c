...
{
	697.0,  770.0,  852.0,  941.0, 2600
};
static double dtmf_col[] =
{
	1209.0, 1336.0, 1477.0, 1633.0, 2600
};
...
static const char dtmf_positions[] = "123A-" "456B-" "789C-" "*0#D-" "----S";
...
{
	double row_energy[DTMF_MATRIX_SIZE];
	double col_energy[DTMF_MATRIX_SIZE];
	double famp;
	int i;
	int j;
	int sample;
	...
	for (sample = 0;  sample < samples;  sample = limit) {
		...
		for (j = sample; j < limit; j++) {
			...
			goertzel_sample(s->td.dtmf.row_out + 4, amp[j]); // 112
			goertzel_sample(s->td.dtmf.col_out + 4, amp[j]);
